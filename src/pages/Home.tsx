  ...
  return (
    <PageContainer>
      ...
      <section className="relative min-h-[92vh] flex items-center bg-brand-primary text-white pt-24 pb-0">
        ...
        >
          <div className="grid grid-cols-2 lg:grid-cols-5 bg-white border border-sand-border-strong rounded-2xl overflow-hidden shadow-none">
            {stats.map((stat, idx) => (
              <div 
                key={stat.label} 
                className={`flex flex-col items-center text-center py-10 px-4 relative border-sand-border/60 ${
                  idx % 2 === 0 ? 'border-r' : ''
                } ${
                  idx < 4 ? 'border-b' : ''
                } ${
                  idx !== 4 ? 'lg:border-r lg:border-b-0' : 'lg:border-r-0 lg:border-b-0'
                }`}
              >
                {stat.icon}
